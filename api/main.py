...
async def upload_csv(file: UploadFile = File(...)):
    ...
    feature_order_lines = [line for line in result.split('\n') if line.startswith("[") and line.endswith("]")]
...
async def predict(sample: SampleInput):
    ...
    if not model_path.exists() or not scaler_path.exists() or not feature_order_path.exists():
        raise HTTPException(status_code=400, detail=f"Model, scaler, or feature order not found for '{target}'")
